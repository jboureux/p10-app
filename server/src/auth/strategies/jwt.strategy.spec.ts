...
describe('JwtStrategy', () => {
  ...
  describe('validate', () => {
    it('should return user data when token is valid', async () => {
      ...
      expect(blacklistService.isBlacklisted).toHaveBeenCalledWith('valid-token');
    });

    it('should throw UnauthorizedException when token is blacklisted', async () => {
      const req = { get: jest.fn().mockReturnValue('Bearer blacklisted-token') };
      const payload = { sub: 1, email: 'test@example.com' };
      ...
      expect(blacklistService.isBlacklisted).toHaveBeenCalledWith('blacklisted-token');
    });
